...
//        player.seek(to: CMTime(seconds: 0, preferredTimescale: 600)) // Numerator/Denominator seconds = N, D = 600
//        player.play()
    ...
    private func bestCamera() -> AVCaptureDevice {
        ...
        preconditionFailure("Tell user: No cameras on device match the specs that we need")
    }
    ...
    private func bestMicrophone() -> AVCaptureDevice {
        ...
        preconditionFailure("No microphones on device")
    }
...
extension CameraViewController: AVCaptureFileOutputRecordingDelegate {
    ...
    func fileOutput(_ output: AVCaptureFileOutput, didFinishRecordingTo outputFileURL: URL, from connections: [AVCaptureConnection], error: Error?) {
        if let error = error {
            print("Tell user: Error saving video: \(error)")
        }
